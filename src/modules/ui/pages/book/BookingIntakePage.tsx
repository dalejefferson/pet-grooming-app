  ...
  return (
    <div className="space-y-6">
      ...
      {selectedPets.length > 0 && !hasAvailableServices && (
        <div className="rounded-2xl border-2 border-[#1e293b] bg-white p-8 text-center shadow-[3px_3px_0px_0px_#1e293b]">
          <p className="text-lg font-semibold text-[#1e293b] mb-2">No Services Available</p>
          <p className="text-sm text-[#64748b] mb-4">
            {selectedGroomer
              ? `${selectedGroomer.firstName} ${selectedGroomer.lastName} doesn't have any compatible services. Please go back and choose a different groomer.`
              : 'No services are currently available. Please try again later.'}
          </p>
          <Button variant="secondary" onClick={() => navigate(`/book/${organization.slug}/groomer`)}>
            Change Groomer
          </Button>
        </div>
      )}
