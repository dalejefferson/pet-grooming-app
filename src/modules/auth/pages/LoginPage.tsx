...
import { Dog, FlaskConical } from 'lucide-react'
...
import { enableTestMode } from '../api/authApi'
...
export function LoginPage() {
  ...
  const handleTestMode = async () => {
    ...
    setError('')
    try {
      await login.mutateAsync({ email: 'admin@pawsclaws.com', password: 'demo123' })
      enableTestMode()
      // Invalidate all queries so SubscriptionProvider re-renders and picks up the test mode flag
      await queryClient.invalidateQueries()
      navigate('/app/dashboard', { replace: true })
    } catch (_err) {
      setError('Test mode login failed. Make sure demo accounts are seeded.')
      ...
      <div className="w-full max-w-sm">
        ...
        <Card>
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-xl border-2 border-dashed border-[#1e293b]/40 bg-[#fef9c3] px-4 py-2 text-sm font-semibold text-[#1e293b] transition-all duration-150 hover:-translate-y-0.5 hover:border-[#1e293b] hover:shadow-[4px_4px_0px_0px_#1e293b] active:translate-y-0 active:shadow-[1px_1px_0px_0px_#1e293b] disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {testModeLoading ? (
            ...
            {testModeLoading ? 'Signing in...' : 'Enter Test Mode'}
            {!testModeLoading && <span className="ml-1 rounded-md bg-[#1e293b]/10 px-1.5 py-0.5 text-xs font-medium">Full Access</span>}
          </button>
